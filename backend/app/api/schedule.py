...
def patch_study_time(study_time_id):
    """
    PATCH /api/schedule/study-times/:id
    Body: { start_time?, end_time?, notes? } (ISO format for times).
    """
    auth = request.headers.get("Authorization")
    payload = decode_token(auth)
    if not payload:
    ...
    try:
        ...
        cur.execute(
            """
            SELECT st.id, st.term_id FROM StudyTimes st
            JOIN Terms t ON t.id = st.term_id
        ...
        if "notes" in data:
            ...
            params.append(data.get("notes") or None)
