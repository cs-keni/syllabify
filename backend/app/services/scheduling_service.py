...
def _reconcile_deadlines(session: Session, user_id: int, assignments: list) -> dict[int, datetime]:
    ...
    for assignment in assignments:
        if getattr(assignment, Assignment.due_date.key, None) is None:
            continue
...
def generate_study_times(
    ...
    - No conflict with course meetings, calendar events, or locked study times.
    - Respects max_hours_per_day and preferred_days.
    - Cost structure encourages balanced load across days.
    ...
    # Skip assignments with no due_date (no window to schedule into).
    assignments = [
        a for course in term.courses for a in course.assignments
        if getattr(a, Assignment.due_date.key, None) is not None
    ]
    ...
    # Use user's timezone so 5pm-10pm is correct (not UTC). Use first available date (ongoing always has both; assignments have due_date after filter).
    first_dt = work_items[0].start_date or work_items[0].due_date
    if user_timezone:
        try:
    ...
    # Single phase: min-cost max-flow. Balances load across days and respects all constraints.
    return _generate_study_times_global(
        session=session,
        term=term,
...
class _MinCostMaxFlow:
    ...
    def add_edge(self, fr: int, to: int, cap: int, cost: int) -> None:
        fwd = _MCFEdge(to, len(self.g[to]), cap, cost)
        rev = _MCFEdge(fr, len(self.g[fr]), 0, -cost)
        self.g[fr].append(fwd)
        self.g[to].append(rev)

    def min_cost_flow(self, s: int, t: int, max_f: int) -> tuple[int, int]:
        """Successive shortest augmenting path with potentials (non-negative reduced costs)."""
        n = self.n
        inf_cost = 10**18
        h = [0] * n  # potentials
        prevv = [0] * n
...
def _generate_study_times_global(
    ...
    """
    Global scheduling using min-cost max-flow.
    - If a conflict-free, fully allocated schedule exists within the term's
      study window, meetings, calendar events, locked blocks, preferred days,
      and max_hours_per_day, this will find one.
    - Among all such schedules, it prefers more balanced day usage by giving
      increasing marginal cost to additional slots on the same day.
    """
    # Normalize assignment windows to the same timezone and compute total demand.
    # Skip assignments with no due_date (no study times generated for them).
    # If due_date is set but start_date is None, assume start = due - 1 week.
    ...
    for a in assignments:
        ...
        we = (
            ...
            else getattr(a, Assignment.due_date.key, None)
        )
        if we is None:
            continue
        ws = getattr(a, Assignment.start_date.key, None)
        if ws is None:
    ...
    effective_day_caps: list[int] = []
    for count in slots_per_day:
        effective_day_caps.append(min(count, max_slots_per_day_cap))

    # If we cannot satisfy demand within the daily caps, return empty.
    if sum(effective_day_caps) < total_required_slots:
        return []
    ...
    mcf = _MinCostMaxFlow(node_count)

    # Source -> assignments (capacity = workload in slots).
    for i, (a, _ws, _we) in enumerate(normalized_assignments):
        if a.work_load <= 0:
            continue
        mcf.add_edge(source, assign_offset + i, a.work_load, 0)

    # Assignment -> slots (each edge capacity 1, cost 0).
    for i in range(len(normalized_assignments)):
        from_node = assign_offset + i
        for slot_idx in assignment_slot_indices[i]:
            mcf.add_edge(from_node, slot_offset + slot_idx, 1, 0)

    # Slot -> day (each slot belongs to exactly one day, capacity 1, cost 0).
    for slot_idx, (_s, _e, di) in enumerate(all_slots):
        mcf.add_edge(slot_offset + slot_idx, day_offset + di, 1, 0)

    # Day -> sink: multiple unit-capacity edges with increasing cost per additional slot
    # on that day. This induces a convex cost that encourages balancing.
    for di, cap in enumerate(effective_day_caps):
        for k in range(cap):
            mcf.add_edge(day_offset + di, sink, 1, k)

    flow, _ = mcf.min_cost_flow(source, sink, total_required_slots)
    if flow < total_required_slots:
        # Not enough capacity to fully satisfy demand under constraints.
        return []
    ...
    # Merge adjacent 15-min slots into contiguous blocks so we create one StudyTime per block.
    ...
    for a, _ws, _we in normalized_assignments:
        ...
        merged = _merge_intervals(intervals)  # adjacent (e.g. 9:00-9:15, 9:15-9:30) become one
